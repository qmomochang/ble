...
public class CsGetLatestSyncStatusRequestTask extends CsConnectivityTask {

    private final static String TAG = "CsGetLatestSyncStatusRequestTask";

    private BluetoothDevice mBluetoothDevice;

    public CsGetLatestSyncStatusRequestTask(CsBleTransceiver csBleTransceiver, Messenger messenger, ExecutorService executor, BluetoothDevice device) {
    ...
    public void execute() throws Exception {
        ...
        inArray[0] = (byte) 0x00;
        futureA0 = mExecutor.submit(new CsBleReceiveNotificationCallable(mCsBleTransceiver, mBluetoothDevice, CsBleGattAttributes.CsV1CommandEnum.LATEST_SYNC_STATUS_EVENT));
        futureA1 = mExecutor.submit(new CsBleSetNotificationCallable(mCsBleTransceiver, mBluetoothDevice, CsBleGattAttributes.CsV1CommandEnum.LATEST_SYNC_STATUS_EVENT, true));

        if (futureA1.get() == null) {

            sendMessage(false, null);
            unregisterNotify(CsBleGattAttributes.CsV1CommandEnum.LATEST_SYNC_STATUS_EVENT);
            return;
        }

        futureB = mExecutor.submit(new CsBleWriteCallable(mCsBleTransceiver, mBluetoothDevice, CsBleGattAttributes.CsV1CommandEnum.LATEST_SYNC_STATUS_REQUEST, inArray));
        ...
        if (result != null) {
            ...
            if (result != null)
            {
                byte [] retArray = CsBleGattAttributeUtil.getLatestSyncStatus(result);
    ...
    private void sendMessage(boolean result, byte [] retArray) {
        ...
        try {
            ...
            outData.putByteArray(ICsConnectivityService.PARAM_CS_LATEST_SYNC_STATUS_RESULT, retArray);
